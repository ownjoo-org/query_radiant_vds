...
@pytest.mark.asyncio
async def test_pagination_respects_result_limit() -> None:
    ...
    with patch("query_radiant_vds.client.get_response", side_effect=mock_get_response_side_effect):
        results = []
...
@pytest.mark.asyncio
async def test_pagination_no_limit_returns_all() -> None:
...
@pytest.mark.asyncio
async def test_page_size_optimization() -> None:
    ...
    with patch(
        ...
        assert call_args[1]["params"]["pageSize"] == 5
